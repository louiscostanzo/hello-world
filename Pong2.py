...
# this is a branch that will give me solo pong, it diverges from the tutorial
# Solo pong will speed up a little on every miss, and will be scored only against the player
# perhaps it will reset at 10
...
def make_object(obj_new,width,length,posX,posY,shape,color,speed):
    ...
    return
ball = turtle.Turtle()
make_object(ball,1,1,0,0,'circle','white',ball_speed)
...
pen.hideturtle()
pen.goto(0,260)
pen.write('Computer:0', align='center', font=('Courier',20,'normal'))
...
wn.onkeypress(reset_game, 'y' or 'Y')
...
while game_over == False:
    ...
    if ball.xcor() < -390:
        ...
        score_a += 1
        pen.clear()
        pen.write('Computer:{}'.format(score_a), align='center', font=('Courier',20,'normal'))
    if score_a >= 1:
        pen.clear()
